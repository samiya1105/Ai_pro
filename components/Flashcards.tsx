...
const Flashcards: React.FC = () => {
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center h-full">
        ...
        <p className="text-slate-500">Creating flashcards for {topic}...</p>
      </div>
    );
  }
  ...
  if (cards.length === 0) {
    return (
      <div className="max-w-xl mx-auto mt-20 p-8 bg-white rounded-2xl shadow-sm border border-slate-100">
        <h2 className="text-2xl font-bold text-slate-800 mb-4">Study Flashcards</h2>
        <p className="text-slate-500 mb-6">Enter a subject to generate a deck of flashcards instantly.</p>
  ...
  return (
    <div className="max-w-2xl mx-auto h-full flex flex-col items-center justify-center py-8">
      <div className="w-full flex justify-between items-center mb-6">
        <h3 className="text-lg font-semibold text-slate-700">{topic}</h3>
      ...
      >
        <div className={`relative w-full h-full duration-500 preserve-3d transition-transform ${isFlipped ? 'rotate-y-180' : ''}`}>
          ...
          <div className="absolute w-full h-full backface-hidden bg-white border border-slate-200 rounded-3xl shadow-xl flex flex-col items-center justify-center p-8 text-center">
            <span className="text-xs uppercase font-bold text-slate-400 mb-4 tracking-wider">Question</span>
            <p className="text-2xl font-medium text-slate-800">{cards[currentIndex].front}</p>
      ...
      <div className="flex items-center gap-6 mt-10">
        <button 
          onClick={prevCard}
          className="p-4 bg-white rounded-full shadow-md text-slate-600 hover:bg-slate-50 transition-colors"
        >
          <ChevronLeft size={24} />
        </button>
        <button
          ...
          className="px-6 py-3 rounded-xl font-medium text-slate-500 hover:text-slate-800 transition-colors"
        >
          New Deck
        </button>
        <button 
